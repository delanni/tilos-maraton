...
performances.forEach((performance) => {
    ...
    if (start.getHours() < 8) {
        if (supposedDayIdForEarlyActs !== dayId) {
            console.log(`Morning-performance ${performance.id} is not on the correct day: ${dayId} (should be ${supposedDayIdForEarlyActs})`);
        }
    } else {
        if (supposedDayId !== dayId) {
            console.log(`Performance ${performance.id} is not on the correct day: ${dayId} (should be ${supposedDayId})`);
        }
    }

    const supposedIdStart = `06-${(5 + parseInt(day)).toString().padStart(2, "0")}`;
    if (!id.startsWith(supposedIdStart)) {
        console.log(`Performance ${performance.id} is not on the correct day: ${dayId} (should be ${supposedIdStart})`);
    ...
    if (duration > 5 * 60 * 60 * 1000 && !performance.id.includes("heti-betevo")) {
        console.log(`Performance ${performance.id} is longer than 5 hours: ${duration / 1000 / 60 / 60} hours`);
    }
});
...
Object.entries(performancesPerDay).forEach(([day, performances]) => {
    ...
    const lastPerformancePerStage = Object.fromEntries(
        Object.entries(performancesPerStage).map(([stage, performances]) => {
            const performancesSorted = performances.sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime());
    ...
    Object.entries(lastPerformancePerStage).forEach(([stage, performance]) => {
        ...
        if (duration < 2 * 60 * 60 * 1000) {
            console.log(`Performance ${performance.id} on stage ${stage} on day ${day} is not long enough: ${duration / 1000 / 60 / 60} hours`);
            performance.endTime = toIsoWithOffset(new Date(new Date(performance.startTime).getTime() + 2 * 60 * 60 * 1000));
        }
    });
});
...
const artistsPresentInPerfNotInArtistsJson = performances.filter(p => !artistIdsInArtistsJson.includes(p.artistId));

if (artistsPresentInPerfNotInArtistsJson.length > 0) {
    console.log("Artists present in performances.json but not in artists.json:", artistsPresentInPerfNotInArtistsJson.map(p => p.artistId));
}

const artistsPresentInArtistsJsonNotInPerf = artistsJson.filter(a => !artistIdsInPerformanceJson.includes(a.id));

if (artistsPresentInArtistsJsonNotInPerf.length > 0) {
    console.log("Artists present in artists.json but not in performances.json:", artistsPresentInArtistsJsonNotInPerf.map(a => a.id));
}
...
const caravanProgrammesWithoutSimilarMessage = caravanProgrammes.filter(p => !p.description.includes("Élő közvetítés a Dürer Kertből"));
if (caravanProgrammesWithoutSimilarMessage.length > 0) {
    console.log("Not all caravan programmes have a similar message:", caravanProgrammesWithoutSimilarMessage.map(p => p.id));
...
if (Object.keys(performancesById).length !== performances.length) {
    console.log("Duplicate performances:", Object.keys(performancesById).filter(id => performancesById[id] !== performances.find(p => p.id === id)));
}
...
const days = performances.map(p => p.dayId).filter((value, index, self) => self.indexOf(value) === index);
const tombolas = performances.filter(p => p.artistId.includes("tombola"));
if (days.length !== tombolas.length) {
    console.log("Not every day has a tombola:", days.filter(day => !tombolas.some(p => p.dayId === day)));
}
