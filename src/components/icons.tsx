import tilosLogo from "../../resources/logo.svg";
export function TilosLogo({
  className,
}: {
  className?: string;
}) {
  return (
    <div
      className={`bg-main hover:bg-accent flex items-center border border-border transition-colors rounded-full px-2 py-0 ${className}`}
    >
      <img src={tilosLogo} alt="Tilos Logo" />
    </div>
  );
}
...
export function HeartIcon({
  ...
  return (
    <svg
      ...
      className={`h-5 w-5 ${filled ? "fill-current text-red-500" : "text-current"} ${className}`}
      viewBox="0 0 20 20"
...
export function YoutubeIcon() {
  return (
    <svg className="h-6 w-6 mx-2" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      <path
        fillRule="evenodd"
...
export function DefaultArtistIcon() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className="h-16 w-16"
...
export const missingArtistFallbackIcon =
